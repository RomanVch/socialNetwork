...
const [message, setmessage] = useState(props.PropsMessage[props.id])
    ...
    const onChangeHandler =(e:ChangeEvent<HTMLInputElement>)=>{debugger
        props.upAddMessage(e.currentTarget.value) }

    const addMessage = () => {
props.addMessage(props.id,today)
}

return (
    <div className={style.dialogs__block_message}>

        <input onChange={onChangeHandler} value={props.txtMsg}  placeholder="Что нового ?" className={style.dialogs__block_message_input} type="text"  />
        <div className={style.dialogs__block_message_buttons}>
            <button type="button" className={style.dialogs__block_message_button}><img width="40px" height="40px" src={clip} /></button>
            <button type="button" className={style.dialogs__block_message_button}><img width="40px" height="40px" src={music} /></button>
            <button type="button" onClick={addMessage}  className={style.dialogs__block_message_button}><img width="40px" height="40px" src={send} /></button>
        </div>
